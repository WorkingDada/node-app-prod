      ...
      steps{
        ...
        script {
          dir("${WORKSPACE}/app-config") {
            sh "rm -f production.properties || true"
          ...
          withKubeConfig([credentialsId: 'KubeConfigCred', serverUrl: 'https://minikubeCA:58431']) {
            sh 'kubectl create configmap appconfig --from-file=${WORKSPACE}/app-config/production.properties -n techx-trainee-production -o yaml --dry-run=client | kubectl apply -f -'
          }
        }
      }
    }   
    ...
    stage('Deploying App to Kubernetes') {
      steps {
        script {
          echo "--- Deploying ---"
          withKubeConfig([credentialsId: 'KubeConfigCred', serverUrl: 'https://minikubeCA:58431']) {
            sh 'kubectl apply -f ${WORKSPACE}/app-code/deploymentservice.yml -n techx-trainee-production'
          }
        }
      }
    }
